...
export const evolutiveAchievements = {
  checkAndEvolve(req: Request, res: Response): void {
    ...
    try {
      ...
      res.status(500).json({ message: 'Failed to check evolution', error: String(error) });
    }
  },
  ...
  getUserEvolutiveAchievements(req: Request, res: Response): void {
    const { userId, guildId } = req.params;
    try {
      const achievements = service.getUserEvolutiveAchievements(userId, guildId);
      res.json({ data: achievements });
    } catch (error) {
      res.status(500).json({ message: 'Failed to get evolutive achievements', error: String(error) });
    }
  },
  ...
  getEvolutionTimeline(req: Request, res: Response): void {
    ...
    try {
      ...
      res.status(500).json({ message: 'Failed to get evolution timeline', error: String(error) });
    }
  },
};
