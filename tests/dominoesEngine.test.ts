...
import {
  DominoesEngine,
  type Tile,
} from '../src/services/activity/dominoesBlock/DominoesEngine';
...
describe('DominoesEngine deal', () => {
  ...
  it('deals the full double-six set with no duplicates across hands and boneyard', () => {
    ...
    const state = engine.getState();
    const all = [...state.hands.a, ...state.hands.b, ...state.hands.c, ...state.boneyard];
    expect(all).toHaveLength(28);
    const ids = new Set(all.map((t) => `${Math.min(t.a, t.b)}-${Math.max(t.a, t.b)}`));
  ...
  it('starts with the player holding the highest double', () => {
    ...
    expect(state.players).toContain(state.currentPlayer);
    if (highestDoubleOwner) {
      ...
      if (anyoneHasDouble) expect(doublesInStartingHand.length).toBeGreaterThan(0);
    }
  });
});

describe('DominoesEngine.playTile', () => {
  function engineWithHands(hands: Record<string, Tile[]>, boneyard: Tile[] = []) {
    ...
    (engine as any).state.hands = hands;
    (engine as any).state.boneyard = boneyard;
    (engine as any).state.currentPlayer = players[0];
    return engine;
  }
  ...
  it('requires an end once the chain is non-empty', () => {
    ...
    (engine as any).state.currentPlayer = 'a';
  ...
  it('extends the right end, orienting the tile so the matching pip touches', () => {
    ...
    expect(result.success).toBe(true);
    const state = engine.getState();
    expect(state.chain).toEqual([{ a: 3, b: 5 }, { a: 5, b: 6 }]);
  ...
  it('extends the left end, orienting the tile so the matching pip touches', () => {
    ...
    expect(result.success).toBe(true);
    const state = engine.getState();
    expect(state.chain).toEqual([{ a: 2, b: 3 }, { a: 3, b: 5 }]);
...
describe('DominoesEngine.pass', () => {
  ...
  it('shares the win between tied pip totals on a block', () => {
    ...
    (engine as any).state.hands = {
      a: [{ a: 6, b: 6 }],
      b: [{ a: 6, b: 6 }],
    };
    (engine as any).state.chain = [{ a: 0, b: 0 }];
    (engine as any).state.leftEnd = 0;
    (engine as any).state.rightEnd = 0;
    (engine as any).state.currentPlayer = 'a';
...
describe('DominoesEngine.getPlayableTiles', () => {
  it('treats every tile as playable before the chain starts', () => {
    const engine = new DominoesEngine(['a', 'b'], { rng: () => 0 });
    (engine as any).state.hands.a = [{ a: 1, b: 2 }, { a: 3, b: 4 }];
  ...
  it('lists both ends when a tile matches both open values', () => {
    const engine = new DominoesEngine(['a', 'b'], { rng: () => 0 });
    (engine as any).state.chain = [{ a: 3, b: 3 }];
    (engine as any).state.leftEnd = 3;
    (engine as any).state.rightEnd = 3;
    (engine as any).state.hands.a = [{ a: 3, b: 5 }, { a: 0, b: 1 }];
    const playable = engine.getPlayableTiles('a');
